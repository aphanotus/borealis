...
What's happening behind the scenes is that `borealis` relies heabily on the function in the R package `geomorph`. The output of the `procrustes.alignment` function is similar to the object type produced by `geomorph::gpagen`. However it also includes data provenance.

### Outlier detection

After inital GPA, it's typically important to remove any obvious outliers. These may be specimens where coordinate positions were recorded incorrectly. `procrustes.alignment` fasciliates interactive outlier detection and removal. Just include the arguement `outlier.analysis = TRUE`.
