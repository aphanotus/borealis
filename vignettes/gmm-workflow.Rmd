...
data provenance, some visualizations, and greater user flexibililty.
...
![](https://i.imgur.com/p3MKN81.png)


By default, the function will plot a random specimen, with the new landmark positions. 
Here we have specified which one to plot using the arguement `show.plot = 1`. 
The second plot is a histogram of mean distances amonmg landmarks (corrected for centroid size).
This can be used as a metric of how well the alignment "improves" the consistency of landmark positions. However, there are some expections to this interpretation. 
If the alimgnment "stretches out" the sub-structure for some specimens, this may produce an increase in mean landmark distances that swamps any reduction caused by greater consistency among the landmarks of that structure. 
Consider the histogram and mean changes, but use your best judgement.
